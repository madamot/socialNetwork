<?php
  ...
  if (isset($_POST['text'])) {
    $text = sanitzeString($_POST['text']);

    if ($text != "") {
      $pm = substr(sanitzeString($_POST['pm']),0,1);
      $time = time();
  ...
  if ($view != "") {
    if ($view == $user) {
      ...
      $name1 = "<a href='mambers.php?view=$view'>$view</a>'s";
      $name2 = "$view's";
    }
    ...
    if (isset($_GET['erase'])) {
      $erase = sanitzeString($_GET['erase']);
